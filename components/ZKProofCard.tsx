  ...
  return (
    <section className="rounded-lg border border-line bg-panel p-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">ZK 风控证明</p>
          <h2 className="mt-2 text-xl font-semibold">隐私因子已验证</h2>
        </div>
        <span className="rounded-md bg-aqua/10 px-3 py-2 text-sm text-aqua">隐私保护中</span>
      </div>
      ...
      <div className="mt-5 space-y-3">
        {checks.map((check) => (
          <div key={check.label} className="rounded-md bg-black/20 px-3 py-3">
